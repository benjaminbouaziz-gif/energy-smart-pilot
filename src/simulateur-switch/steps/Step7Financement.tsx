...
import {
  Banknote, Wallet, TrendingUp, CheckCircle2, FileDown, Mail, Loader2,
  ChevronLeft,
} from "lucide-react";
...
export default function Step7Financement() {
  const { data, prev, prospectId } = useSimulateurSwitch();
  ...
  const client = {
    nom:
      data.identite?.nomEntreprise ||
      `${data.identite?.prenom ?? ""} ${data.identite?.nom ?? ""}`.trim() ||
      "Client",
    email: data.identite?.email ?? "",
    telephone: data.identite?.telephone ?? "",
    pdl: data.switchgrid?.prm ?? "",
    adresse: data.identite?.adresse ?? "",
  };

  const suffix = "TTC";
  const fmtMode = (n: number) => fmt(n);

  const reportContainerRef = useRef<HTMLDivElement | null>(null);
  const [reportPayload, setReportPayload] = useState<any | null>(null);
  const [generatingPdf, setGeneratingPdf] = useState(false);
  const [mode, setMode] = useState<Mode>("leasing");
  const [duree, setDuree] = useState<60 | 84>(60);
  const [saved, setSaved] = useState(false);

  const handleDownloadReport = async () => {
    if (!result) { toast.error("Aucune simulation à exporter"); return; }
    if (generatingPdf) return;
    setGeneratingPdf(true);
    const payload = { client, facture, result, date: new Date().toISOString() };
  ...
  const cashflow = useMemo(() => {
    ...
    for (let m = 1; m <= horizon; m++) {
      const flux = gainMensuelTtc - (mode === "leasing" && m <= duree ? loyerTtc : 0);
      cumul += flux;
      ...
      const { error } = await supabase
        ...
        .update({
          resultats_simulation: {
            ...
            monthlyData: (result as any).monthlyData,
            annualTotals: (result as any).annualTotals,
            coverageMonths: (result as any).coverageMonths,
            isFullYear: (result as any).isFullYear,
            full: result,
          } as any,
        })
        .eq("id", prospectId);
  ...
  return (
    <>
      ...
      >
        ...
        {mode === "leasing" && (
          <div className="flex items-center justify-center gap-2 mb-6">
            <span className="text-xs font-mono uppercase tracking-widest text-muted-foreground mr-2">Durée</span>
        ...
        <div className="grid md:grid-cols-3 gap-4 mb-8">
          <Kpi label={`Gain mensuel ${suffix}`} value={fmtMode(gainMensuelTtc)} sub="économies pilotage Dynawatt" tone="gold" />
          {mode === "leasing" ? (
            <Kpi label={`Loyer mensuel ${suffix}`} value={fmtMode(loyerTtc)} sub={`sur ${duree} mois`} tone="muted" />
          ) : (
            <Kpi label="Investissement" value={fmtMode(prixTtcEff)} sub="payé une seule fois" tone="muted" />
          )}
          <Kpi
        ...
        <div className="glass rounded-3xl p-5 md:p-7 mb-6">
          <div className="text-[10px] font-mono uppercase tracking-widest text-gold mb-1">Vue mensuelle</div>
        ...
        <div className="glass rounded-3xl p-5 md:p-7 mb-8">
          ...
          {breakevenMois ? (
            ...
            <p className="text-sm text-muted-foreground mb-4">Cashflow positif dès le premier mois.</p>
          )}
          <div className="h-72">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={cashflow}>
                ...
                {breakevenMois && (
                  <ReferenceLine x={breakevenMois} stroke="hsl(var(--accent))" strokeDasharray="4 4"
                    label={{ value: "Break-even", fill: "hsl(var(--accent))", fontSize: 10 }} />
                )}
                <Line type="monotone" dataKey="cumul" stroke="hsl(var(--primary))" strokeWidth={3} dot={false} />
              </LineChart>
            </ResponsiveContainer>
          </div>
        </div>
      ...
      <div className="container mx-auto px-4 mt-6 mb-10 max-w-5xl flex justify-start">
      ...
      {reportPayload && (
        <div
          aria-hidden
          style={{
            position: "fixed", left: "-100000px", top: 0, width: "210mm",
...
function Kpi({
  ...
  tone: "gold" | "primary" | "muted"; highlight?: boolean;
}) {
  ...
  return (
    ...
    >
      <div className={`text-[10px] font-mono uppercase tracking-widest mb-2 ${color}`}>{label}</div>
