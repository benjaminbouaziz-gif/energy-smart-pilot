...
import { SimulatorProvider, useSimulator } from "@/simulatorbis/SimulatorContext";
import { WizardHeader } from "@/simulatorbis/components/WizardHeader";
import { supabase } from "@/integrations/supabase/client";
import { toast } from "sonner";
import Step1Client from "@/simulatorbis/steps/Step1Client";
import Step2Upload from "@/simulatorbis/steps/Step2Upload";
import Step3Config from "@/simulatorbis/steps/Step3Config";
import Step4Comparison from "@/simulatorbis/steps/Step4Comparison";
import Step5Animations from "@/simulatorbis/steps/Step5Animations";
import Step6Financing from "@/simulatorbis/steps/Step6Financing";
...
function WizardBody({ prospectId }: { prospectId?: string }) {
  ...
  return (
    <div className="simulator-light relative min-h-screen pb-16 bg-background text-foreground">
      <div
        className="border-b px-4 py-2 text-sm flex items-center justify-center gap-2 font-semibold"
        style={{ background: "#FED7AA", color: "#7C2D12", borderColor: "#FB923C" }}
      >
        <FlaskConical className="w-4 h-4" />
        <span>🧪 Mode test — Simulateur version bis</span>
      </div>
...
export default function Simulateurbis() {
